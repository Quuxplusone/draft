...
\\
then \tcode{xprime} is \tcode{tuple_cat(tuple<allocator_arg_t, polymorphic_allocator&>(allocator_arg, *this), std::move(x))}.
\item
Otherwise, if \tcode{uses_allocator_v<T1,polymorphic_allocator>} is \tcode{true}
\\
and
...
\\
then \tcode{xprime} is \tcode{tuple_cat(std::move(x), tuple<polymorphic_allocator&>(*this))}.
\item
Otherwise the program is ill formed.
\end{itemize}
...
\\
then \tcode{yprime} is \tcode{tuple_cat(tuple<allocator_arg_t, polymorphic_allocator&>(allocator_arg, *this), std::move(y))}.
\item
Otherwise, if \tcode{uses_allocator_v<T2,polymorphic_allocator>} is \tcode{true}
\\
and
...
\\
then
\tcode{yprime} is \tcode{tuple_cat(std::move(y), tuple<polymorphic_allocator&>(*this))}.
\item
Otherwise the program is ill formed.
\end{itemize}

Then, using \tcode{piecewise_construct}, \tcode{std::move(xprime)}, and \tcode{std::move(yprime)}
