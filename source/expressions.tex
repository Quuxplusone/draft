...
\pnum
The closure type for a \grammarterm{lambda-expression} with no
\grammarterm{lambda-capture} has a defaulted default constructor.
\enternote Its copy constructor, move constructor, copy assignment operator,
and move assignment operator are implicitly defined as usual.
Since the closure type has no non-static data members, all of its
special member functions are trivial. \exitnote

\pnum
The closure type for a \grammarterm{lambda-expression} with a
\grammarterm{lambda-capture} has no
