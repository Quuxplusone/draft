...
\pnum
A \term{value pack} is the result of applying the
postfix \tcode{\~{}} operator to a \grammarterm{postfix-expression}.
The number of arguments contained by a value pack~\tcode{expr\~{}} is
defined as \tcode{std::tuple_size_v<decltype(expr)>}. \enterexample

\begin{codeblock}
void f(int, double);
std::tuple<int, double> t { 1, 2.0 };

f(t~ ...);          // OK: \tcode{t\~{}} contains two arguments: \tcode{1} and \tcode{2.0}
\end{codeblock}
...
pack, a function parameter pack, or a value pack.
...
the instantiation of the pattern where the pack is declared; otherwise,

\item
if the pack is a value pack, the element is the \grammarterm{postfix-expression}
\tcode{std::get<i>(p)}, where \tcode{p} is the
\grammarterm{postfix-expression} obtained by removing the
unpacking operator~\tcode{\~{}} from the value pack.
\end{itemize}
